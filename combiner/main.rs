...
enum ImageDataErrors {
    ...
    UnableToReadImageReaderFromPath(std::io::Error),
...
fn main() -> Result<(), ImageDataErrors> {
    ...
    if let Err(e) = image::save_buffer_with_format(output.name, &output.data, output.width, output.height, image::ColorType::Rgba8, image_1_format) {
...
fn find_image_from_path(path: String) -> Result <(DynamicImage, ImageFormat), ImageDataErrors> {
    ...
    match Reader::open(&path) {
        ...
        Err(e) => Err(ImageDataErrors::UnableToReadImageReaderFromPath(e))
    }
}
